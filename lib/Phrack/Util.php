...
class Phrack_Util
{
    ...
    {
        return new Phrack_Util_Headers($headers);
    }
...
class Phrack_Util_Prototype
{
    ...
    public function __call($attr, $args)
    {
        if (isset($this->callbacks[$attr]) && is_callable($this->callbacks[$attr])) {
            ...
            throw new Exception(
                "Can't locate object method \"$attr\" via package \"Phrack_Util_Prototype\"");
        }
    }
}

class Phrack_Util_Headers
{
    protected $headers;
    ...
    {
        return Phrack_Util::headerIter($this->headers, $callback);
    }

    public function get($key)
    {
        return Phrack_Util::headerGet($this->headers, $key);
    }

    public function set($key, $value)
    {
        return Phrack_Util::headerSet($this->headers, $key, $value);
    }

    public function push($key, $value)
    {
        return Phrack_Util::headerPush($this->headers, $key, $value);
    }

    public function exists($key)
    {
        return Phrack_Util::headerExists($this->headers, $key);
    }

    public function remove($key)
    {
        return Phrack_Util::headerRemove($this->headers, $key);
    }

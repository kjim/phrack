...
function testCallApp($t)
{
    ...
    $env = array();
    list($status, $headers, $body) = Phrack_Util::callApp($app, $env);
...
{
    $func = array('Phrack_Util', 'contentLength');
...
function testHeaderIter($t)
{
    ...
    Phrack_Util::headerIter($headers, 'callback');
...
function testHeaderGet($t)
{
    ...
    $cl = Phrack_Util::headerGet($headers, 'Foo');
...
{
    $headers = array(array('Foo', 'bar'));
    Phrack_Util::headerSet($headers, 'Bar', 'baz');
    $t->is_deeply($headers, array(array('Foo', 'bar'), array('Bar', 'baz')));

    $headers = array(array('Foo', 'bar'));
    Phrack_Util::headerSet($headers, 'Foo', 'baz');
...
{
    $headers = array(array('Foo', 'bar'));
    Phrack_Util::headerPush($headers, 'Bar', 'baz');
    Phrack_Util::headerPush($headers, 'Bar', 'qux');
    $locations = Phrack_Util::headerGet($headers, 'Bar');
...
{
    $headers = array(array('Foo', 'bar'));
    $exists = Phrack_Util::headerExists($headers, 'Foo');
    $t->ok($exists, 'headerExists() returns true value');

    $exists = Phrack_Util::headerExists($headers, 'Bar');
...
{
    $headers = array(array('Foo', 'bar'));
    Phrack_Util::headerRemove($headers, 'Foo');

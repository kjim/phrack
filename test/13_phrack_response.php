...
function testNew($t)
{
    $res = new Phrack_Response(302);
    $t->is($res->getStatus(), 302);

    // array_array syntax
    $res = new Phrack_Response(200, array(array('Content-Type', 'text/plain')));
    $t->is($res->getContentType(), 'text/plain');

    // hash syntax
    $res = new Phrack_Response(200, array('Content-Type' => 'text/plain'));
    $t->is($res->getContentType(), 'text/plain');

    $res = new Phrack_Response(200);
